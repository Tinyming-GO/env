...
cat <<EOF
------------------------------------------------------------------------------
$(sudo apt-get install figlet -y --no-install-recommends >/dev/null>&1 && figlet " Mario Ding Shell")
  Author:Mario Ding  Email:2267957534@qq.com
------------------------------------------------------------------------------
EOF
}
...
# if [ ${UID} == 0 ]; then
    ...
    case "$iChoose" in
        ...
        403)
        showDoingTask "oh-my-zsh" && sudo apt-get install zsh -y && sudo chsh -s $(which zsh) && \
        sudo apt-get install git && \
        wget -c -P $(pwd) https://raw.githubusercontent.com/robbyrussell/oh-my-zsh/master/tools/install.sh && \
        sudo sh $(pwd)/install.sh && rm -f $(pwd)/install.sh && showResultTip "oh-my-zsh"
        ;;
        ...
        sudo make install-config && cd .. && rm -rf proxychains-ng && showResultTip "Proxychains4"
        ;;
